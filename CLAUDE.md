...
- `data/scenario.json`: valori nazionali correnti. È l'UNICO file da riscrivere per l'aggiornamento periodico. Campi: `version` (intero, incrementare), `updatedAt` (ISO), `updatedLabel` (data in italiano), `supermedia` {`date` ISO, `label` in italiano, `url` dell'articolo, `lists` nome lista → percentuale}, `previous` {`date`, `label`, `lists`} con la Supermedia precedente, `adjustments` {CasaRif, Centro}, `targets` (chiavi esattamente FdI, PD, M5S, FN, FI, AVS, Lega, CasaRif, Centro, NM, Altri; somma 100), `prevTargets` (stesse chiavi, valori della Supermedia precedente: servono alle frecce di variazione in home), `coalitions` (perimetri, NON cambiare finché non saranno depositate le liste ufficiali).
  Ad ogni aggiornamento: PRIMA copiare `supermedia` in `previous` e `targets` in `prevTargets`, POI scrivere i nuovi valori.
